...
from main_app.choices import LaptopBrandChoice, LaptopOSChoice
...
# Import your models
from main_app.models import ArtworkGallery, Workout, Dungeon, Meal, ChessPlayer, Laptop
...
def update_to_512_GB_storage():
    Laptop.objects.filter(brand__in=[LaptopBrandChoice.ASUS, LaptopBrandChoice.LENOVO]).update(storage=512)


def update_to_16_GB_memory():
    Laptop.objects.filter(brand__in=[LaptopBrandChoice.APPLE, LaptopBrandChoice.DELL, LaptopBrandChoice.ACER]).update(memory=16)


def update_operation_systems():
    Laptop.objects.update(
        operation_system=Case(
            When(brand=LaptopBrandChoice.ASUS, then=Value(LaptopOSChoice.WINDOWS)),
            When(brand=LaptopBrandChoice.APPLE, then=Value(LaptopOSChoice.MACOS)),
            When(brand__in=[LaptopBrandChoice.DELL, LaptopBrandChoice.ACER], then=Value(LaptopOSChoice.LINUX)),
            When(brand=LaptopBrandChoice.LENOVO, then=Value(LaptopOSChoice.CHROME_OS))
        )
    )
...
def show_hard_dungeons() -> str:
    return '\n'.join(f"{d.name} is guarded by {d.boss_name} who has {d.boss_health}"
                     f" health points!" for d in Dungeon.objects.filter(difficulty='Hard').order_by('-location'))
